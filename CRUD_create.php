...
$brand_name = $brewer = $origin = $beer_style = $spec_beer_style = $description = $food_pairings = $abv = $ibu = $srm = $calories = $ounces = $milliliters = "";
$brand_name_err = $brewer_err = $origin_err = $beer_style_err = $spec_beer_style_err = $description_err = $food_pairings_err = $abv_err = $ibu_err = $srm_err = $calories_err = $ounces_err = $milliliters_err = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($input_spec_beer_style)) {
        $spec_beer_style_err = "Please enter a specific beer style.";
    } else {
    ...
    if (empty($input_food_pairings)) {
        $food_pairings_err = "Please enter food pairings.";
    } else {
    ...
    if (empty($input_calories)) {
        ...
        $calories_err = "Please enter a valid calories value.";
    } else {
        $calories = $input_calories;
    }
    
    // Check input errors before inserting in database
    if (empty($brand_name_err) && empty($brewer_err) && empty($origin_err) && empty($beer_style_err) && empty($spec_beer_style_err) && empty($description_err) && empty($food_pairings_err) && empty($abv_err) && empty($ibu_err) && empty($srm_err) && empty($calories_err)) {
        // Prepare an insert statement
        $sql = "INSERT INTO beers (brand_name, brewer, origin, beer_style, spec_beer_style, description, food_pairings, abv, ibu, srm, calories, ounces, milliliters) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
         
        if ($stmt = mysqli_prepare($link, $sql)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssssssiiiii", $param_brand_name, $param_brewer, $param_origin, $param_beer_style, $param_spec_beer_style, $param_description, $param_food_pairings, $param_abv, $param_ibu, $param_srm, $param_calories, $param_ounces, $param_milliliters);
